<?php
	 ...
	 *
	 * INPUT:  object view, object user
	 * OUTPUT: -
	 * ERROR:  -
	 */
	function prevent_csrf($view, $user) {
		...
		if (isset($_SERVER["HTTP_ORIGIN"]) != false) {
			...
			if ($_SESSION["csrf_warning_shown"] == false) {
				$view->add_system_warning("Your browser hides the referrer HTTP header line. You are therefor vulnerable for CSRF attacks via this website!");
		...
		$view->add_system_warning($message, $_SERVER["HTTP_REFERER"]);
	 ...
	 * ERROR:  -
	 */
	function secure_password($password, $view) {
		$result = true;
